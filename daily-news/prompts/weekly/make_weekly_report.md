...
- `correlate_iocs.py` は `daily-news/scripts/ioc_shared_service_hosts.json` を使い、GitHub / Telegram / CDN / Cloudflare Workers などの正規共有サービスについては `host_overlap` を抑制する
- ただし `exact` match は抑制しない
- 抑制された shared service host overlap を、週次レポートで関連候補として再表示しない
