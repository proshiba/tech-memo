...
また、このRoutineでは週次レポート本文に加えて、IOC分析も行います。
...
週次レポート作成時にAIがIOC元CSVを直接すべて読み込んで突合しないこと。
過去IOCとの突合は、必ず `daily-news/scripts/correlate_iocs.py` の出力CSVを使ってください。

### shared service host list

IOC host overlap のノイズ抑制用に、以下のリストファイルを使います。

```text
daily-news/scripts/ioc_shared_service_hosts.json
```

このファイルに記載された GitHub / Telegram / CDN / Cloudflare Workers などの正規共有サービスホストは、`host_overlap` の突合から除外されます。
ただし、`exact` match は除外されません。
...
- daily Markdown
- IOC元CSV
- `daily-news/scripts/correlate_iocs.py`
- `daily-news/scripts/ioc_shared_service_hosts.json`
- unrelated files
...
python3 daily-news/scripts/correlate_iocs.py \
  ...
  --shared-service-hosts-file daily-news/scripts/ioc_shared_service_hosts.json
```
...
4. shared service host overlap の抑制状況
   - `host_overlap_suppressed_current_shared_service_rows`
   - `host_overlap_suppressed_past_shared_service_rows`
   - GitHub / Telegram / CDN / Cloudflare Workers などの共有サービスが host_overlap から除外されているか
...
### shared service host overlap の扱い

`correlate_iocs.py` は `daily-news/scripts/ioc_shared_service_hosts.json` を使い、正規共有サービスホストの `host_overlap` を抑制します。
...
これらは、host が同じだけでは関連候補として扱わないこと。

ただし、完全一致の `exact` match は抑制しないこと。
つまり、同じ GitHub URL が過去にも出ている場合は `exact` として表示してよい。
...
- `ioc_shared_service_hosts.json` により抑制された host_overlap を、手動で復元して表示しないこと
- shared service host で一致していないことを「関連なし」と断定しないこと。単に host_overlap のノイズを抑制しているだけと扱うこと
- exact match と host_overlap を混同しないこと
...
12. IOC分析スクリプトの出力CSVを読み、IOCサマリと過去IOC突合結果を追加する
...
- suppressed current shared-service host_overlap rows
- suppressed past shared-service host_overlap rows

`{{week_label}}_ioc_summary.csv` に該当する metric がある場合はそれを使うこと。
無い場合は、読み込んだ `current_iocs` / `ioc_matches` から計算できる範囲だけ出すこと。

抑制件数の metric 名は以下を使うこと。
...
- `host_overlap_suppressed_current_shared_service_rows` / `host_overlap_suppressed_past_shared_service_rows` が多い場合、GitHub・CDN・共有ホスティング等によるノイズが多かった可能性がある
- 抑制された host_overlap は、週次レポートでは関連候補として列挙しない
- exact match は共有サービス上であっても残るため、表示された exact match は個別に確認する
...
- shared service host の `host_overlap` を関連候補として再表示していない
- `exact` match と `host_overlap` match を混同していない
- IOCサマリに shared service host overlap の抑制件数を反映している
...
- shared service host list を使ったか
- 今週の IOC rows
- distinct IOC count
- IOC past match 件数
...
では、対象期間の日次CSV群を読み、IOC分析を行い、週次レビュー Markdown を作成または更新してください。
